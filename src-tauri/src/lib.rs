...
#[tauri::command]
async fn get_local_mods(game_path: Option<String>) -> Result<Vec<ModInfo>, String> {
    let project_root = get_data_dir();
...
fn mod_root_for(gltf_path: &Path) -> Option<std::path::PathBuf> {
...
const EMPTY_ASSETS: ModAssets = ModAssets { props: Vec::new(), textures: Vec::new() };
...
fn collect_mod_assets(root: &Path) -> ModAssets {
...
fn find_props_file(assets: &ModAssets, gltf_dir: &Path, mat_name: &str) -> Option<std::path::PathBuf> {
...
fn find_texture_file(assets: &ModAssets, gltf_dir: &Path, stem: &str) -> Option<std::path::PathBuf> {
...
fn texture_uri(tex_path: &Path, gltf_dir: &Path) -> String {
...
fn relative_uri(from_dir: &Path, file: &Path) -> String {
...
/// `body_n.png` do not).
fn is_diffuse_name(stem: &str) -> bool {
...
/// if absent.
fn set_material_basecolor(mat: &mut serde_json::Value, tex_idx: usize) {
...
fn build_standalone_gltf(gltf_path: &Path) -> Option<std::path::PathBuf> {
...
fn refresh_standalone(cached: std::path::PathBuf) -> std::path::PathBuf {
...
fn get_unique_dir(base_dir: &Path, name: &str) -> PathBuf {
...
#[tauri::command]
fn clear_mod_cache() -> Result<(), String> {
...
#[tauri::command]
fn save_collections_json(collections_json: String) -> Result<(), String> {
    let file_path = get_data_dir().join("collections.json");
    fs::write(file_path, collections_json).map_err(|e| e.to_string())
}

#[tauri::command]
fn load_collections_json() -> Result<String, String> {
    let file_path = get_data_dir().join("collections.json");
    if file_path.exists() {
        fs::read_to_string(file_path).map_err(|e| e.to_string())
    } else {
...
#[tauri::command]
fn prepare_import(file_paths: Vec<String>) -> Result<Vec<PrepareImportResult>, String> {
    use sha2::{Sha256, Digest};
    use std::io::Read;
...
#[tauri::command]
fn create_dir_if_not_exists(path: String) -> Result<(), String> {
